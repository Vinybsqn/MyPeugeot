...
export default function StatsRow({ data }) {
  const stats = [
    {
      icon: <Thermometer size={15} style={{ color: 'var(--t2)' }} />,
      value: data?.environment?.air?.temp != null ? `${data.environment.air.temp}°C` : '--',
      label: 'Temp.',
    },
    {
      icon: <Gauge size={15} style={{ color: 'var(--t2)' }} />,
    ...
    {
      icon: <Mountain size={15} style={{ color: 'var(--t2)' }} />,
    ...
    <div className="grid grid-cols-4 gap-2">
      {stats.map((s, i) => (
        <div key={i} className="card flex flex-col items-center gap-2 py-4 px-2">
          {s.icon}
          <span className="text-sm font-bold" style={{ color: 'var(--t1)' }}>{s.value}</span>
